		...
		<div class="row">
			<div class="col-sm-12">
				<div class="panel">
					...
					<!--Block Styled Form -->
					<!--===================================================-->
					<form method="POST" action="{{ route('dashboard.students.update', $user->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
							...
							<div class="row">
								<div class="col-sm-12">
									<div class="form-group">
										...
										<input name="serial" value="{{ $user->student->serial ?? old('serial') }}" type="text" class="form-control" required>						
									</div>
									@error('serial')
								...
								<div class="col-sm-6">
									<div class="form-group">
										...
										<select name="status" class="form-control" required>
											<option @if ($user->student->status == 'hold') selected @endif value="hold">hold</option>
											<option @if ($user->student->status == 'accepted') selected @endif value="accepted">accepted</option>
											<option @if ($user->student->status == 'graduated') selected @endif value="graduated">graduated</option>
										</select>									
									</div>
									@error('status')
										<span class="invalid-feedback" role="alert">
								...
								<div class="col-sm-6">
									<div class="form-group">
										...
										<select name="blood_type" class="form-control" required>
											<option @if ($user->student->blood_type == 'A+') selected @endif value="A+">A+</option>
											<option @if ($user->student->blood_type == 'A-') selected @endif value="A-">A-</option>
											<option @if ($user->student->blood_type == 'B+') selected @endif value="B+">B+</option>
											<option @if ($user->student->blood_type == 'B-') selected @endif value="B-">B-</option>
											<option @if ($user->student->blood_type == 'AB+') selected @endif value="AB+">AB+</option>
											<option @if ($user->student->blood_type == 'AB-') selected @endif value="AB-">AB-</option>
											<option @if ($user->student->blood_type == 'O+') selected @endif value="O+">O+</option>
											<option @if ($user->student->blood_type == 'O-') selected @endif value="O-">O-</option>
										</select>									
									</div>
									@error('blood_type')
